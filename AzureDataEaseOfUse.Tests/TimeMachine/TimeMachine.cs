...
{
    // ref: https://github.com/tekpub/DemoCode/blob/master/AsyncIntro/Code/Testing.NUnit/TimeMachine.cs

    public sealed class TimeMachine
    {
        private readonly SortedDictionary<int, Action> actions = new SortedDictionary<int, Action>();

        public Task<T> ScheduleSuccess<T>(T value)
        {
            int time = actions.Count > 0 ? actions.Keys.Max() + 1 : 1;
        ...
        public Task<T> ScheduleFault<T>(Exception exception)
        {
            int time = actions.Count > 0 ? actions.Keys.Max() + 1 : 1;
        ...
        public Task<T> ScheduleCancellation<T>()
        {
            int time = actions.Count > 0 ? actions.Keys.Max() + 1 : 1;
        ...
        public class Advancer
        {
            ...
            public void AdvanceTo(int targetTime)
            {
                ...
                Thread.Sleep(100); // prevent ContinueWith, chained tasks from failing execution
            }
            ...
            public void AdvanceToCurrent()
            {
                if (actions.Count == 0)
                    return;

                AdvanceTo(actions.Keys.Max());
            }
        }
    }
